<#
===========================================================
Driver Governance — V13-Lite (Enhanced v3)
===========================================================
...
# Optional self-test (set environment variable DRIVERGOV_SELFTEST=1 to run).
<# Placed here so all functions above are defined and available.
if ($env:DRIVERGOV_SELFTEST -eq '1') {
    Write-Output "DriverGov self-test starting..."
    ...
    $sample = [PSCustomObject]@{
        ...
        InfName = 'oem1.inf'
        DriverDate = (Get-Date).AddDays(-1)
        IsSigned = $true
        DeviceClass = 'Network'
    }

    $sig = Get-DriverSignatureLevel $sample
    $risk = Get-DriverRiskScore $sample $sig 'OEM/Manual'
    $post = Detect-PostInstallOptimized $sample $ApprovedVendors @() @() @{} @()

    Write-Output "Self-test results:"
    Write-Output " Signature: $sig"
    Write-Output " Risk: $($risk.Score) ($($risk.Tier))"
    Write-Output " PostInstall detection: $([string]::IsNullOrWhiteSpace(($post | Out-String)) ? 'None' : ($post | Out-String))"
    exit 0
}#>
...
function Get-DriverRiskScore {
    ...
    # ========== FACTOR 3: VENDOR REPUTATION (policy-driven)
    $vendor = Normalize-VendorAdvanced $Driver.DriverProviderName
    $vendorRiskModifier = 0

    # Consult governance config RiskTiers first (policy-driven)
    $vendorTier = $null
    try {
        if ($GovernanceConfig -and $GovernanceConfig.RiskTiers -and $GovernanceConfig.RiskTiers[$vendor]) {
            $vendorTier = $GovernanceConfig.RiskTiers[$vendor]
        }
    } catch {}

    switch ($vendorTier) {
        'trusted' { $vendorRiskModifier = -15; $reasons += "$vendor vendor (-15, trusted by policy)" }
        'oem'     { $vendorRiskModifier = -5;  $reasons += "$vendor OEM (-5, policy)" }
        'unknown' { $vendorRiskModifier = 10;  $reasons += "$vendor vendor (+10, unknown in policy)" }
        default   {
            # Fallback mapping for older hardcoded behavior when RiskTiers not configured
            if ($vendor -eq 'Microsoft') { $vendorRiskModifier = -15; $reasons += "Microsoft vendor (-15, trusted)" }
            elseif ($vendor -in @('Intel','AMD','NVIDIA','Broadcom')) { $vendorRiskModifier = -10; $reasons += "$vendor vendor (-10, trusted)" }
            elseif ($vendor -in @('Dell','HP','Lenovo')) { $vendorRiskModifier = -5; $reasons += "$vendor OEM (-5, known)" }
            else { $vendorRiskModifier = 10; $reasons += "$vendor vendor (+10, unverified)" }
        }
    }
...
function Get-DriverSignatureLevel {
    ...
    # Prefer platform-provided signed indicator from Win32_PnPSignedDriver when available
    try {
        if ($d -and $d.PSObject.Properties['IsSigned']) {
            if ($d.IsSigned) { return "Valid" }
            # If IsSigned is present and false, fall through to authenticode enrichment
        }
    }
    catch {}

    # Fallback to INF-based Authenticode enrichment (cached)
    if (-not $d.InfName) { return "Unknown" }

    return Get-CachedSignature -InfName $d.InfName
}
...
function Resolve-InstallSourceOptimized {
    ...
    $WUClassPatterns = @("System","Processor|CPU","Storage.*Controller","NET|Network","USB Host Controller","HID")
    $classMatch = $WUClassPatterns | Where-Object { $Driver.DeviceClass -match $_ }
    if ($classMatch -and $Driver.DriverProviderName -and $Driver.DriverProviderName -notmatch "Custom|OEM|Vendor" -and 
        $Driver.DriverProviderName -match "Standard|Compatible") { return "Windows Update" }

    if ($WUHistoryLookup -and $WUHistoryLookup.Count -gt 0 -and $Driver.DriverProviderName -and $WUHistoryLookup.ContainsKey($Driver.DriverProviderName)) {
...
function Detect-PostInstallOptimized {
    ...
    $vendor = Normalize-VendorAdvanced $d.DriverProviderName
    $deviceClass = $d.DeviceClass
    # Use the unified signature level (prefers Win32_PnPSignedDriver.IsSigned, falls back to cached Authenticode)
    $sig = Get-DriverSignatureLevel $d
    $recent = Get-RecentDriverInstall $d
...
foreach($d in $drivers){
    # Prefer PostInstall detection first; suppress Policy alerts when a PostInstall exists for the same driver
...
# ============ DETECTION SUMMARY ============
Write-Output ""
Write-Output "=== Pre-remediation detection output ==="
Write-Output "V13(Enhanced v3)"
Write-Output ""

# Statistics
$totalScanned = if ($pending) { $pending.Count } else { 0 }
$totalScanned += if ($drivers) { $drivers.Count } else { 0 }
Write-Output "Total scanned: $totalScanned (Pending: $(if ($pending) { $pending.Count } else { 0 }), Installed: $(if ($drivers) { $drivers.Count } else { 0 }))"
Write-Output "Issues detected: $($results.Count)"

if ($results -and $results.Count -gt 0) {
    $criticalCount = @($results | Where-Object { $_.PSObject.Properties['Severity'] -and $_.Severity -eq "Critical" }).Count
    $highCount = @($results | Where-Object { $_.PSObject.Properties['Severity'] -and $_.Severity -eq "High" }).Count
    $mediumCount = @($results | Where-Object { $_.PSObject.Properties['Severity'] -and $_.Severity -eq "Medium" }).Count
    $lowCount = @($results | Where-Object { $_.PSObject.Properties['Severity'] -and $_.Severity -eq "Low" }).Count
    
    Write-Output "Breakdown: Critical=$criticalCount, High=$highCount, Medium=$mediumCount, Low=$lowCount"
    Write-Output ""
    
    # Show details of detected issues
    Write-Output "=== Detected Issues ==="
    $results | ForEach-Object {
        $stage = if ($_.PSObject.Properties['Stage']) { $_.Stage } else { "Unknown" }
        ...
        $severity = if ($_.PSObject.Properties['Severity']) { $_.Severity } else { "Unknown" }
        $message = if ($_.PSObject.Properties['Message']) { $_.Message } else { "" }
        
        Write-Output "[$stage] ${severity}: $driver [$vendor] v$version - $message"
    }
    Write-Output ""
}
